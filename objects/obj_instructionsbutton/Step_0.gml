...
You must paste the string from the export button into ZeroRanger's set_palette() script, using a tool like UndertaleModTool (https://github.com/krzys-h/UndertaleModTool/releases/latest) which I'll be explaining how to use here.
If you know how GML and UndertaleModTool work, You could probably do this without reading the rest here. If you don't: Keep reading.
Before you begin, it's very important you finish the game first to unlock palette selection. You can still do this guide by overriding the colorblind palette (ID 1) 
but I suggest you just beat it first.
...
If you would like to add a palette, simply add a new Switch case with the next free ID
above the default case and paste the export button text to it (Remember to add a break statement! if you don't, you'll just get default colors). Then you must go to
the object controller_options' User Event 2 (aka Other_12) to around line 180, and you need to modify this bit of code:

if (global.realm_reseted <= 0)
	global.palette = wrap_fully((global.palette + seldir), 0, 2)
else
	global.palette = wrap_fully((global.palette + seldir), 0, 6)
	
This code is for cycling through palettes in the options menu.
The first set of lines check if you have beaten the game, so modify the one that is relevant to you.
The second set of lines set the global.palette variable to wrap between 2 numbers. 
In order to uncap the amount of palettes you must set the last argument (2 or 6) to the amount of palettes minus one. So if you added just one palette, that would be 32.
So you'd change it to:
global.palette = wrap_fully((global.palette + seldir), 0, 32)
	
from there you can save the data.win like in the other method, and if you've done everything correctly, you're done!
